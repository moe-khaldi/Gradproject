...
export default function AITeachingSystem() {
  ...
  const formatReferences = (references) => {
    ...
    return references.map((ref) => {
      ...
      const parts = [ref.source || 'Unknown source'];
      if (ref.page) {
        parts.push(`page ${ref.page}`);
      }
      if (ref.chunk_id) {
        parts.push(`chunk ${ref.chunk_id}`);
      }
      return parts.join(', ');
    });
  };
  ...
  const handleSendMessage = async (message, actionType = null) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to contact the backend service.');
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: 'The backend RAG service is currently unavailable. Please check that the Django server is running and the OpenAI and Qdrant settings are configured.',
        timestamp: new Date().toISOString(),
        error: true
      }]);
      return;
  ...
  const generateQuiz = async (config = {}) => {
    try {
      ...
      setError(err.message || 'Failed to generate quiz from the backend service.');
      return;
  ...
  const UploadPage = () => {
    ...
    const handlePasteSubmit = async () => {
      ...
      try {
        ...
        setError(err.message || 'Failed to explain pasted content.');
        return;
        console.error('❌ Explanation failed:', err);
        // Fallback
        setCurrentPage('chat');
